...
from models.equifm.cnf import Cnflows
...
def get_model(args, device, dataset_info, dataloader_train):
    ...
    net_dynamics = EGNN_dynamics_QM9(
        ...
        normalization_factor=args.normalization_factor, aggregation_method=args.aggregation_method, cfg=args.cfg, rep_dropout_prob=args.rep_dropout_prob, rep_nf=args.rep_nf, attn_dropout=args.attn_dropout, attn_block_num=args.attn_block_num
         )
    ...
    elif args.probabilistic_model == 'fm':
        efm = Cnflows(
            dynamics=net_dynamics,
            in_node_nf=in_node_nf,
            n_dims=3,
            ...
            include_charges=args.include_charges,
            discrete_path=args.get('discrete_path', "OT_path"),
            cat_loss=args.get('cat_loss', "l2"),
            cat_loss_step=args.get('cat_loss_step', -1),
            on_hold_batch=args.get('on_hold_batch', -1),
            sampling_method=args.get('sampling_method', "vanilla"),
            weighted_methods=args.get('weighted_methods', "jump"),
            ode_method=args.get('ode_method', "dopri5"),
            without_cat_loss=args.get('without_cat_loss', False),
            angle_penalty=args.get('angle_penalty', False),
        )

        return efm, nodes_dist, prop_dist
