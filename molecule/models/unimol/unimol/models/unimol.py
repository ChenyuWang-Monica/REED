...
class UniMolModel(BaseUnicoreModel):
    ...
    def forward(
        ...
        assert self.global_condition == (
                    input_global_rep is not None), "Global condition and clean global rep should be both True or False"

        if input_global_rep is not None:
            x = x + self.node_feature_fusion(
                torch.cat([x, input_global_rep.unsqueeze(1).expand(-1, x.size(1), -1)], dim=-1))
