...
class Cnflows(torch.nn.Module):
    ...
    def phi(self, t, x, node_mask, edge_mask, context, rep=None):
        ...
        net_out = self.dynamics._forward(t, x, node_mask, edge_mask, context, rep=rep)
    ...
    def decode(self, z, node_mask, edge_mask, context, rep=None):
        def wrapper(t, x):
            dx = self.phi(t, x, node_mask, edge_mask, context, rep=rep)
    ...
    def decode_chain(self, z, t, node_mask, edge_mask, context, rep=None):
    ...
    def sample_cat_z0(self, xh, node_mask, edge_mask, context, rep=None):
        """
        ...
        net_out = self.phi(0.0, xh, node_mask, edge_mask, context, rep=rep)
        z_h = net_out[
              :, :, self.n_dims: -1
    ...
    def sample(
        ...
        z_ = self.decode(z, node_mask, edge_mask, context, rep=rep_context)[-1]
        ...
        if self.sampling_method == "gradient":
            ...
            for i_ in categorical_steps:
                # slightly perturb
                gradient = self.phi(
                    torch.tensor([i_]), z_, node_mask, edge_mask, context, rep=rep_context
                )
    ...
    def sample_chain(
        ...
        chain_z = self.decode_chain(z, time_step, node_mask, edge_mask, context, rep=rep)
