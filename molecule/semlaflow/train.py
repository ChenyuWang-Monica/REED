...
from semlaflow import scriptutil as util
...
from semlaflow.rep_samplers import *
...
# Try locally import if globally import failed
...
def build_model(args, dm, vocab):
    ...
    if args.arch == "semla":
        dynamics = EquiInvDynamics(
            ...
            rep_alignment=args.rep_alignment,
        ...
        from flowmodels.eqgat import EqgatGenerator
        ...
        from flowmodels.egnn import VanillaEgnnGenerator
    ...
    if args.diffusion_loss_max_epoch > 0:
        diffusion_loss_max_step = util.calc_train_steps(dm, args.diffusion_loss_max_epoch, args.acc_batches)
    else:
        diffusion_loss_max_step = -1
    ...
    # from flowmodels.wrapper import SelfConditionWrappedSampler
    ...
    fm_model = MolecularCFM(
        ...
        type_mask_index=type_mask_index,
        bond_mask_index=bond_mask_index,
        rep_condition=args.rep_condition,
        encoder=encoder,
        rdm=rep_sampler,
        ...
        align_local=args.align_local,
        diffusion_loss_max_step=diffusion_loss_max_step,
        **hparams
    )
    return fm_model
...
def build_dm(args, vocab):
    ...
    test_dataset = GeometricDataset.load(data_path / "test.smol", transform=transform)
...
if __name__ == "__main__":
    ...
    # representation condition & alignment
    parser.add_argument("--rep_condition", default=False, action='store_true')
    parser.add_argument("--d_rep", type=int, default=DEFAULT_D_REP)
    parser.add_argument("--encoder_type", type=str, default='unimol_global')  # 'unimol', 'unimol_global' or 'frad'
    parser.add_argument("--encoder_path", type=str, default='./checkpoints/unimol_global.pt')  
    parser.add_argument("--cond_type", type=str, default='none')  # 'adaLN', 'cross_attn' or 'none'
    parser.add_argument("--finetune_encoder", default=False, action="store_true")
    parser.add_argument("--noise_sigma", type=float, default=0.0)  # larger noise sigma is better for DRUGS; 0.5 may be too big
    parser.add_argument("--rep_alignment", default=False, action="store_true")
    parser.add_argument("--repa_loss_weight", type=float, default=1.0)
    parser.add_argument('--align_depth', nargs='+', type=int, default=[4])
    parser.add_argument('--align_local', default=False, action="store_true")
    ...
    parser.add_argument("--diffusion_loss_max_epoch", type=int, default=-1)
    ...
    # RDM Sampling - not used in REPA.
    # Note this can be changed after training during evaluation. We set it here just to monitor the trajectory.
    parser.add_argument("--sampler", type=str, default='None')  # ["PCSampler", "GtSampler", "DDIMSampler", "None"]
    #     For DDIMSampler and PCSampler
    parser.add_argument("--rdm_ckpt", type=str, default='./checkpoints/rdm_ckpts/unimol-huge-checkpoint-77.pth')  # resume from checkpoint
