...
class MolecularCFM(L.LightningModule):
    def __init__(
        ...
        noise_sigma=0.3,
        rep_alignment: bool = False,
        repa_loss_weight: float = 1.0,
        align_local: bool = False,
        diffusion_loss_max_step: int = -1,
        **kwargs
    ):
        super().__init__()
        ...
        self.current_step = 0
        self.diffusion_loss_max_step = diffusion_loss_max_step
        self.type_mask_index = type_mask_index
        self.bond_mask_index = bond_mask_index
        ...
        self.rep_alignment = rep_alignment
        self.repa_loss_weight = repa_loss_weight
        self.align_local = align_local
    ...
    def training_step(self, batch, b_idx):
        ...
        rep = get_global_representation(mask, self.encoder, coords, atom_types, training_encoder=self.finetune_encoder,
                                        noise_sigma=self.noise_sigma, dataset=self.dataset, align_local=self.align_local) if self.rep_condition else None
        if rep is not None:
            assert (rep.requires_grad and self.finetune_encoder) or (not rep.requires_grad and not self.finetune_encoder)
        z_rep = get_global_representation(mask, self.encoder, coords, atom_types, training_encoder=False,
                                          noise_sigma=self.noise_sigma, dataset=self.dataset, align_local=self.align_local) if self.rep_alignment else None
            ...
            if torch.rand(1).item() > 0.5:
                with torch.no_grad():
                    ...
                    if self.rep_alignment:
                        cond_coords, cond_types, cond_bonds, _, zs = out
                    else:
                        cond_coords, cond_types, cond_bonds, _ = out
                    cond_batch = {
                        "coords": cond_coords,
        ...
        if self.rep_alignment:
            coords, types, bonds, charges, zs = out
        else:
            coords, types, bonds, charges = out
        predicted = {
            "coords": coords,
            "atomics": types,
            "bonds": bonds,
        ...
        if self.rep_alignment:
            proj_loss = 0.
            z_rep = torch.nn.functional.normalize(z_rep, dim=-1)
            for z_j in zs:
                z_j = torch.nn.functional.normalize(z_j, dim=-1)
                proj_loss += -(z_j * z_rep).sum(dim=-1)
            proj_loss /= len(zs)
            losses["repa"] = proj_loss.mean() * self.repa_loss_weight
    ...
    def _distill_training_step(self, batch):
        ...
        if self.rep_alignment:
            ...
            for z_j in zs:
                ...
                proj_loss += -(z_j * z_rep).sum(dim=-1)
            proj_loss /= len(zs)
            loss = loss + proj_loss.mean() * self.repa_loss_weight
    ...
    def _loss(self, data, interpolated, predicted):
        ...
        if self.diffusion_loss_max_step > 0:
            diffusion_scale = min(self.current_step / self.diffusion_loss_max_step, 1.0)
            coord_loss *= diffusion_scale
            type_loss *= diffusion_scale
            bond_loss *= diffusion_scale
            charge_loss *= diffusion_scale
    ...
    def _generate(self, prior, steps, strategy="linear"):
        ...
        if self.rep_condition and self.rdm is not None:
            self.rdm.eval()
        ...
        with torch.no_grad():
            for step_size in step_sizes:
                ...
                out = self(curr, times, training=False, cond_batch=cond, rep=sampled_rep)
                if self.rep_alignment:
                    coords, type_logits, bond_logits, charge_logits, zs = out
                else:
                    coords, type_logits, bond_logits, charge_logits = out
