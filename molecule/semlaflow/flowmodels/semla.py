...
from flowmodels.attention import BasicTransformerBlock, DiTBlock, TimestepEmbedder
...
class EquiMessagePassingLayer(torch.nn.Module):
    def __init__(
        ...
        if self.d_rep is not None and self.cond_type != "none":
...
class EquiInvDynamics(torch.nn.Module):
    ...
    def forward(self, coords, inv_feats, adj_matrix, atom_mask=None, edge_feats=None, cond_coords=None, rep=None, times=None):
        ...
        if self.d_edge is None:
            if self.rep_alignment:
                return out_coords, inv_feats, zs
            return out_coords, inv_feats

        edge_feats = self.bond_norm(edge_feats)
        if self.rep_alignment:
            return out_coords, inv_feats, edge_feats, zs
...
class SemlaGenerator(MolecularGenerator):
    ...
    def forward(
        ...
        if not self.rep_alignment:
            ...
            if len(out) == 3:
                pred_coords, pred_feats, zs = out
            elif len(out) == 4:
                pred_coords, pred_feats, pred_edges, zs = out
        ...
        if self.rep_alignment:
            return pred_coords, type_logits, charge_logits, zs
