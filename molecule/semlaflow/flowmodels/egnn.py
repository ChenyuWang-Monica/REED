...
class VanillaEgnnDynamics(torch.nn.Module):
    def __init__(self, d_model, n_layers, d_edge, d_rep=None, rep_alignment=False, align_depth=[6], d_projector=None):
        super().__init__()
        ...
        if self.rep_alignment:
            ...
            self.projectors = nn.ModuleList([
                build_mlp(d_model, d_projector or d_model, d_rep) for _ in range(len(align_depth))
            ])
    ...
    def forward(self, coords, inv_feats, adj_matrix, atom_mask=None, edge_feats=None, rep=None):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.rep_alignment and i in self.align_depth:
                zs.append(self.projectors[aligned](inv_feats.sum(dim=1)))  # here we use sum pooling to keep align with encoders
                aligned += 1
        ...
        if self.rep_alignment:
            return coords, inv_feats, pred_edges, zs
...
class VanillaEgnnGenerator(MolecularGenerator):
    def __init__(
        ...
        hparams = {
            ...
            "d_projector": d_projector or d_model
        }

        super().__init__(**hparams)

        self.feat_proj = torch.nn.Linear(n_atom_feats, d_model)
        self.dynamics = VanillaEgnnDynamics(d_model, n_layers, d_edge, d_rep=d_rep, rep_alignment=rep_alignment,
                                            align_depth=align_depth, d_projector=d_projector or d_model)
    ...
    def forward(
        ...
        if not self.rep_alignment:
            pred_coords, pred_feats, pred_bonds = pred
        else:
            pred_coords, pred_feats, pred_bonds, zs = pred
        ...
        if self.rep_alignment:
            return pred_coords, type_logits, edge_logits, charge_logits, zs
