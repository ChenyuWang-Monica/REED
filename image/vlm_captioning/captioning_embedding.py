...
from dataset import CustomDatasetWithCaption
...
def main(args):
    ...
    # Setup data:
    train_dataset = CustomDatasetWithCaption(args.data_dir)
    ...
    for raw_image, x, y, raw_text, idx in tqdm(train_dataloader):
        count = 0
        output_text_clean = raw_text
        full_list = []
        ...
        text_embeds_layer_0 = model_output.hidden_states[0]
        text_embeds_layer_1 = model_output.hidden_states[1]
        text_embeds_layer_15 = model_output.hidden_states[15]
        ...
        for i in idx:
            ...
            text_embeds_dir_layer_0 = img_dir.replace("images", "text_embeds_" + model_name_or_path + "_layer_0").replace(img_ext, ".npy")
            text_embeds_dir_layer_1 = img_dir.replace("images", "text_embeds_" + model_name_or_path + "_layer_1").replace(img_ext, ".npy")
            text_embeds_dir_layer_15 = img_dir.replace("images", "text_embeds_" + model_name_or_path + "_layer_15").replace(img_ext, ".npy")

            os.makedirs(os.path.dirname(text_caption_dir), exist_ok=True)
            os.makedirs(os.path.dirname(text_embeds_dir), exist_ok=True)
            os.makedirs(os.path.dirname(text_embeds_dir_layer_0), exist_ok=True)
            os.makedirs(os.path.dirname(text_embeds_dir_layer_1), exist_ok=True)
            os.makedirs(os.path.dirname(text_embeds_dir_layer_15), exist_ok=True)
            ...
            np.save(text_embeds_dir_layer_0, text_embeds_layer_0[count].mean(dim=0).detach().cpu().float().numpy())
            np.save(text_embeds_dir_layer_1, text_embeds_layer_1[count].mean(dim=0).detach().cpu().float().numpy())
            np.save(text_embeds_dir_layer_15, text_embeds_layer_15[count].mean(dim=0).detach().cpu().float().numpy())
            count += 1
...
def parse_args(input_args=None):
    ...
    # model
    parser.add_argument("--model-name-or-path", type=str, default="Qwen2-VL-7B-Instruct")
