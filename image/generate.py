...
def main(args):
    ...
    model = SiT_models[args.model](
        ...
        repr_detach=args.repr_detach,
        repr_residue=args.repr_residue,
        strict_conditioning=args.strict_conditioning,
        **block_kwargs,
    ).to(device)
    # Auto-download a pre-trained model or load a custom SiT checkpoint from train.py:
    ckpt_path = args.ckpt
    if ckpt_path is None:
        args.ckpt = 'SiT-XL-256-REED-1000000.pt'
    ...
    if args.guidance_high == 1.0:
        ...
        folder_name = f"{model_string_name}-{ckpt_string_name}-size-{args.resolution}-vae-{args.vae}-" \
                  f"cfg-{args.cfg_scale}-seed-{args.global_seed}-{args.mode}-cfg-high-{args.guidance_high}"
    ...
    for _ in pbar:
        ...
        sampling_kwargs = dict(
            ...
            flex_param_gamma=args.flex_param_gamma,
            strict_conditioning=args.strict_conditioning,
        )
        with torch.no_grad():
            if args.mode == "sde":
