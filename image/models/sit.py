...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        use_cfg=False,
        z_dims=[768],
        z_types=['i'], # i for image, t for text
        projector_dim=2048,
        **block_kwargs # fused_attn
        ...
        self.encoder_depth_text = encoder_depth_text
    ...
    def forward(self, x, t, y, inference=True):
        """
        Forward pass of SiT.
        ...
        for i, block in enumerate(self.blocks):
            ...
            if not inference:
                if (i + 1) == self.encoder_depth:
                    if self.encoder_depth_text is None or self.encoder_depth == self.encoder_depth_text:
                        zs = [projector(x.reshape(-1, D)).reshape(N, T, -1) if ztype=='i' else projector(x.mean(dim=1))
                            for projector, ztype in zip(self.projectors, self.z_types)]
                    else:
                        for projector, ztype in zip(self.projectors, self.z_types):
                            if ztype=='i':
                                zs_image = projector(x.reshape(-1, D)).reshape(N, T, -1)
                if self.encoder_depth_text is not None and self.encoder_depth != self.encoder_depth_text and (i + 1) == self.encoder_depth_text:
                    for projector, ztype in zip(self.projectors, self.z_types):
                        if ztype=='t':
                            zs_text = projector(x.mean(dim=1))
        if not inference:
            if self.encoder_depth_text is not None and self.encoder_depth != self.encoder_depth_text:
                zs = [zs_image, zs_text]
        else:
            zs = None
